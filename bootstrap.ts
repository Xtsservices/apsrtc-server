...
export async function ensureDefaultAdmin() {
  ...
  try {
    const [roleResult] = await db.query(`INSERT IGNORE INTO role (name, status, created_at) VALUES (?, ?, ?);`, ['admin', 'active', Math.floor(Date.now() / 1000)]);
    const [userResult] = await db.query(`INSERT IGNORE INTO user (email, phone, country_code, username, firstname, lastname, gender, phone_verified, email_verified, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`, [
      'admin@example.com', '1234567890', '+91', 'admin', 'Admin', 'User', 'male', true, true, 'active', Math.floor(Date.now() / 1000)
    ]);
    ...
    if (user && role) {
      ...
      if (!userRoleArr.length) {
        await db.query(`INSERT INTO userrole (user_id, role_id, status, created_at) VALUES (?, ?, ?, ?);`, [user.id, role.id, 'active', Math.floor(Date.now() / 1000)]);
      ...
      if (!loginArr.length) {
        await db.query(`INSERT INTO login (user_id, password, status, created_at) VALUES (?, ?, ?, ?);`, [user.id, encryptedPassword, 'active', Math.floor(Date.now() / 1000)]);
